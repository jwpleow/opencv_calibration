    ...
    for( i = j = 0; i < nimages; i++ )
    {
        for( k = 0; k < 2; k++ )
        {
            ...
            for( int scale = 1; scale <= maxScale; scale++ )
            {
                ...
                found = findChessboardCornersSB(timg, boardSize, corners,
                    CALIB_CB_ACCURACY | CALIB_CB_MARKER | CALIB_CB_NORMALIZE_IMAGE);
                if( found )
                {
                    if( scale > 1 )
                    {
...
int main(int argc, char** argv)
{
    ...
    string imagelistfn;
    bool showRectified;
    cv::CommandLineParser parser(argc, argv, "{w|14|}{h|9|}{s|0.0174|}{nr||}{help||}{@input|stereo_calib.xml|}");
    if (parser.has("help"))
